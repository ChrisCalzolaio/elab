...
Die Abbildung \ref{fig:heightdistance} verdeutlicht das Vorgehen bei diesem iterativen Verfahren.
Die Panels dieser Darstellung zeigen die Entwicklung von Koordinatenkomponenten eines Schneidenpunktes aufgetragen über den Drehwinkel des Werkzeugs, ausgehend von der Startkonfiguration.
...
Bei allen Konfigurationen des Werkzeugs wird die Höhe des Schneidenpunktes im Werkstückoordinatensystem auf diesem Verlauf liegen.
Mit den rot strichlierten Linie ist die obere beziehungsweise untere Begrenzung der Werkstückausdehnung angedeutet.
...
Der äußere Begrenzung des Werkstücks wird durch eine rot strichlierte Linie dargestellt, die im Wert des Radius des Werkstück verläuft.
Die Orte, für welche das Engagement des Werkzeugs geprüft wird, sind mit einem grünen Stern markiert.

Man kann erkennen, dass bei diesem Verfahren das Werkzeug das eine Rotation des Werkzeugs mit einer festen Schrittweite erfolgt, bevor Engagement überprüft wird.
...
Das Resultat dieser Iteration ist in Abbildung \ref{fig:heightdistance} der erste Punkt der dunkelroten Linie.
Er markiert die erste Konfiguration bei der sich mindestens ein Punkt der Schneide im Eingriff befindet.
...
Der hohe Aufwand der notwendig ist um in \matlab dieses Ergebnis zu erreichen zeigt, dass eine externe oder nachträgliche Visualisierung der Simulation notwendig.
...
Für die Entwicklung der Simulation bieten sich vier Arten den Fortschritt der Lösung zu visualisieren an.

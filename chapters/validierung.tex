...
Um Vorzeichen- und Drehmittelpunktfehler sowie Fehler in der Anwendungsreihenfolge der Abbildungsmatrizen zu erkennen ist eine qualitative und visuelle Validierung der in Kap.\ \ref{c:herleitung} entwickelten linearen Algebra anhand einer erprobten Methode also sinnvoll.
...
Translationen können ausschließlich entlang der z-Achse erfolgen und Rotationen erfolgen entsprechend ausschließlich um die z-Achse herum.
Es ist also notwendig, den Komponenten zweckmäßige und individuelle Koordinatensysteme aufzuprägen statt alle auf ein gemeinsames Ursprungs-Koordinatensystem zu referenzieren.

Dies kann entweder direkt im CAD-Modell geschehen und das Bauteil wird mit individuellem Bezugssystem exportiert.
Häufig müssen Bauteile aber zwei Koordinatensysteme haben, eines als ihr eigener Ursprung und ein weiteres, welches sie an ein Tochterbauteil in der kinematischen Kette vererben können.
In \simscape besteht die Möglichkeit innerhalb eines ,,File Solid'' Blocks eine Koordinatentransformation zu definieren.
...
Es ist also notwendig, die Konstruktion dieser Ketten von Koordinatensystem geschickt anzugehen.
...
Das Koordinatensystem $\{c\}$ ist dann wiederum translatorisch verschoben, damit der Ursprung auf der Rotationsachse der Werkzeugspindel liegt.
...
Dieses muss dann im nächsten Schritt noch gedreht werden, damit die z-Achse in Richtung der Rotationsachse der A-Achse zeigt.
...
Ausgehend vom Ursprung des Modells, das ,,world''-Koordinatensystem, breiten sich die gleichen zwei Pfade aus, die bereits in \ref{c:maschkinematik} beschrieben wurden.
...
Zwar kann in der grafischen Ausgabe die Sichtbarkeit der Frames eingestellt werden, die Anzeige wird aber schnell unübersichtlich.
Deshalb wurde die Position des PoI durch Einhängen eines Sphere Solid Blocks in einer auffälligen Farbe markiert.

Durch eine Abweichung des PoIs von der Trajektorie kann ein systematischer Fehler direkt erkannt und eine sehr einfache Fehlersuche betrieben werden.
...
Es war also klar, dass das Werkzeug ein Koordinatensystem mit z-Achse in Richtung der Rotationsachse benötigt, damit, wie in Abbildung \ref{fig:hobspindle_coord} gezeigt, das Polar- und kartesische Koordinatensystem vorteilhaft aufeinander liegen.
...
Um ein grundsätzliches Verständnis dafür zu bekommen, wie sich beide Methoden zueinander verhalten, sich den Abstand $d$ der Bahnen zu jedem Zeitpunkt anzusehen.

\begin{equation*}
	d = \sqrt{\left(x_2-x_1\right)^2 + \left(y_2-y_1\right)^2 + \left(z_2-z_1\right)^2}
\end{equation*}

Dieser Abstand wird wieder über den Winkel des Werkzeugs aufgetragen, und ist 0 wenn sich beide Methoden gleich verhalten.
...
Das selbe Verhalten zeigt sich auch bei einer Analyse der Abweichung je Komponente anhand Abb. \ref{fig:xkomps}, wobei hier noch eine tieferfrequente Funktion überlagert zu sein scheint.
...
Trotz dieser unerklärten Abweichung ist die Methode geeignet, da ohnehin nur eine qualitative Aussage gefordert wurde.
